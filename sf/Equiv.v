...
Proof.
  ...
  Case "->".
    ...
    SCase "b / st || false".
      unfold bequiv in Hb.
      simpl in Hb. rewrite Hb in H5. inversion H5.
    SCase "b / st || true".
      assumption.
  Case "<-".
    apply E_IfFalse.
...
Proof.
  ...
  Case "<-".
    inversion H; subst.
    SCase "(BNot b) / st || true".
      simpl in H5. apply negb_true_iff in H5.
      apply E_IfFalse.
        assumption.
        assumption.
    SCase "(BNot b) / st || false".
...
Proof. 
  ...
  Case "<-".
    apply WHILE_true_nonterm in H. inversion H.
    unfold bequiv. intros st''. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  Case "->".
    ...
    unfold aequiv in He. rewrite <- He.
...
Proof. 
  ...
  Case "<-".
    ...
    SCase "first command".
      apply Hc1. assumption.
    SCase "second command".
      apply Hc2. assumption. Qed.
(** [] *)
...
Proof.
  ...
  Case "->".
    ...
    SCase "false branch".
      apply E_IfFalse;
        [rewrite <- Hb | apply Hc2];
        assumption.
  Case "<-".
    inversion H; subst;
      [apply E_IfTrue | apply E_IfFalse];
        [rewrite Hb | apply Hc1 | rewrite Hb | apply Hc2];
        assumption. Qed.
...
Fixpoint optimize_0plus_aexp (e:aexp) : aexp :=
  ...
  | AId X => AId X
  | APlus (ANum 0) e2 => e2
  | APlus e1 e2 => APlus e1 e2
  | AMinus e1 e2 => AMinus e1 e2
  | AMult e1 e2 => AMult e1 e2
  end.
...
Proof.
  ...
  aexp_cases (induction a) Case;
    simpl; try (apply refl_aequiv).
  Case "APlus".
    destruct a1 eqn:Ha1;
      try (apply refl_aequiv).
    SCase "ANum".
      destruct n as [| n']; try (apply refl_aequiv).
      SSCase "n = 0".
        unfold aequiv. intros st. simpl. reflexivity. Qed.
...
Proof.
  ...
  Case "second command".
    ...
    aexp_cases (induction a2) SCase; simpl;
      try (apply refl_aequiv).
    SCase "AId".
      aexp_cases (destruct a1 eqn:Ha1) SSCase.
      destruct (eq_id_dec i1 i); try (apply refl_aequiv).
      unfold aequiv. intros st.
      rewrite <- (aeval_weakening i1 st (ANum n) (aeval st (AId i))).
      simpl.
      assert (H: aeval st (AId i) = aeval (update st i1 (aeval st a1)) (AId i1)).
      SSSCase "proof of assertion".
        rewrite Ha1.
        rewrite <- (aeval_weakening i1 st (ANum n) n).
        simpl. rewrite <- e. rewrite update_eq. rewrite Ha1. simpl.
        rewrite update_eq. reflexivity. apply H1.
      simpl.
      inversion H1.

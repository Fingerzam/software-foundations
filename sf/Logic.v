...
The _proof term_ for an implication [P -> Q] is a _function_ 
that takes evidence for [P] as input and produces evidence for [Q] 
as its output.
*)     
...
(** The meaning of a proposition is given by _rules_ and _definitions_
    ...
    - Sometimes a proposition is declared to be true without substantiating
      evidence.  Such propositions are called _axioms_.  
...
Proof. 
  intros P.
  split.
    Case "left". intros H. apply H.
    Case "right". intros H. apply H. Qed.
...
Proof.
  ...
  split.
    Case "left". intros HP. apply HQR. apply HPQ. apply HP.
    Case "right". intros HR. apply HQP. apply HRQ. apply HR. Qed.
...
Proof.
  ...
  Case "right". apply or_distributes_over_and_2. Qed.
(** [] *)
...
Proof. 
  ...
  Case "b = false". split.
    ...
    SCase "right". destruct c.
      SSCase "c = true". inversion H.
      SSCase "c = false". reflexivity. Qed.
(** [] *)
...
Theorem contrapositive : forall P Q : Prop,
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q HPQ HnQ. unfold not. intros HP. unfold not in HnQ.
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof. 
  intros P. unfold not. intros HP_and_not_P. inversion HP_and_not_P as [HP HnP].
...
Proof.
  unfold peirce. unfold classic. intros Peirce Q. intros HnnQ.
  apply Peirce with (P := Q) (Q := ~ ~ Q). unfold not. unfold not in HnnQ.
  intros H.
Abort.

Theorem excluded_middle_implies_classic : excluded_middle -> classic.
Proof.
  unfold excluded_middle. unfold classic. intros EM. intros P HnnP.
  assert (H1: P \/ ~P). apply EM. inversion H1 as [HP | HnP].
  Case "left". apply HP.
  Case "right". assert (H2: ~P \/ ~~P). apply EM. inversion H2.
    SCase "left". unfold not in HnnP. unfold not in H. apply HnnP in H.
      inversion H.
    SCase "right". inversion H1 as [HP | HnP'].
      SSCase "left". apply HP.
      SSCase "right". unfold not in HnnP. unfold not in HnP.
        apply HnnP in HnP. inversion HnP. Qed.

Theorem classic_implies_peirce : classic -> peirce.
Proof.
  unfold classic. unfold peirce. intros Classic. intros P Q.
  intros HPQP. apply Classic. unfold not. intros HnP. apply HnP.
  apply HPQP. intros HP. apply HnP in HP. inversion HP. Qed.
...
Proof.
  intros P. unfold not. intros H. apply H. right. intros HP.
  assert (H1: P \/ ~P). left. apply HP. apply H in H1. inversion H1.
Qed.

Theorem classic_implies_excluded_middle : classic -> excluded_middle.
Proof.
  unfold classic. unfold excluded_middle. intros Classic. intros P.
  assert (H: ~~ (P \/ ~P)). apply excluded_middle_irrefutable. apply Classic in H.
  apply H. Qed.

Theorem de_morgan_not_and_not_implies_implies_to_or :
  de_morgan_not_and_not -> implies_to_or.
Proof.
  unfold de_morgan_not_and_not. unfold implies_to_or. intros Demorgan.
  intros P Q HPQ. apply Demorgan. unfold not. intros H. inversion H as [H1 H2].
  assert (H3: ~ (~ P /\ ~ Q) -> P \/ Q). apply Demorgan. unfold not in H3.
  assert (HnPnQ: ~P /\ ~Q). split.
  Case "left". unfold not. intros HP. apply H2. apply HPQ. apply HP.
  Case "right". unfold not. apply H2.
  assert (HnP: ~P). unfold not. intros HP. apply H2. apply HPQ. apply HP.
  assert (HnP_or_Q: ~ (P \/ Q)). unfold not. intros HP_or_Q.
  inversion HP_or_Q as [HP | HQ].
  Case "left". unfold not in HnP. apply HnP in HP. inversion HP.
  Case "right". apply H2 in HQ. inversion HQ.
  unfold not in HnP_or_Q. apply HnP_or_Q. apply H3.
  intros HnP_and_nQ. inversion HnP_and_nQ as [HnP' HnQ'].
  apply H1 in HnP. inversion HnP. Qed. (*write only*)

Theorem implies_to_or_implies_excluded_middle :
  implies_to_or -> excluded_middle.
Proof.
  unfold implies_to_or. unfold excluded_middle.
  intros Implies Q. apply or_commut. apply Implies. intros HQ. apply HQ.
Qed.
...
Proof.
  ...
  intros EM. intros P Q. intros H.
  assert (HP_or_nP: P \/ ~P). apply EM.
  assert (HQ_or_nQ: Q \/ ~Q). apply EM.
  inversion HP_or_nP as [HP | HnP].
    Case "left". left. apply HP.
    Case "right". inversion HQ_or_nQ as [HQ | HnQ].
      SCase "left". right. apply HQ.
      SCase "right". assert (HnP_and_nQ: ~P /\ ~Q).
        SSCase "proof of assertion". split. apply HnP. apply HnQ.
        apply H in HnP_and_nQ. inversion HnP_and_nQ. Qed.

Theorem peirce_implies_classic : peirce -> classic.
Proof.
  unfold peirce. intros Peirce. unfold classic. intros P HnnP.
  apply Peirce with (~~P). intros HPnnP. unfold not in HnnP.
  unfold not in HPnnP. apply Peirce with False. intros HnP.
  apply HnnP in HnP. inversion HnP. Qed.
...
Theorem false_beq_nat : forall n m : nat,
     ...
     beq_nat n m = false.
Proof. 
  intros n m n_neq_m. destruct (beq_nat n m) eqn:Hnm.
  Case "true". unfold not in n_neq_m. apply ex_falso_quodlibet.
    apply n_neq_m. apply beq_nat_true. apply Hnm.
  Case "false". reflexivity. Qed.
(** [] *)
...
Proof.
  intros n m H. unfold not. intros Hnm. rewrite -> Hnm in H.
  rewrite <- beq_nat_refl in H. inversion H. Qed.
(** [] *)

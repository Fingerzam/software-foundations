...
Eval compute in (next_weekday (next_weekday saturday)).
   (* ==> tuesday : day *)
Eval compute in (next_weekday wednesday).
...
Fixpoint inc (b : bin) : bin :=
  ...
  | double_plus_one b' => double (inc b')
  end.

Fixpoint binToNat (b : bin) : nat :=
  match b with
  | zero => 0
  | double b' => mult (S (S 0)) (binToNat b')
  | double_plus_one b' => S (mult 2 (binToNat b'))
  end.

Example test_binToNat1: binToNat
                          (double_plus_one
                            (double
                              (double_plus_one zero))) = 5.
Proof. reflexivity. Qed.

Lemma inc_double_eq_double_plus_one :
  forall (b : bin), inc (double b) = (double_plus_one b).
Proof.
  intros b. reflexivity. Qed.

Theorem binToNat_inc_eq_S_binToNat :
  forall (b : bin), beq_nat (binToNat (inc b)) (S (binToNat b)) = true.
Proof.
  intros b. induction b as [| b' | b'].
    reflexivity.
    rewrite -> inc_double_eq_double_plus_one.
      simpl. Admitted.
(** [] *)

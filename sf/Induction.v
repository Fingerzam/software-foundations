...
Proof.
  ...
  Case "M = S m'".
    simpl. rewrite -> IHm'. rewrite <- mult_n_Sm.
    rewrite -> plus_comm. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros n m p. induction n as [| n'].
  Case "n = 0". rewrite -> mult_0_l. rewrite -> mult_0_l. reflexivity.
  Case "n = S n'".
...
Fixpoint bin_to_nat (b : bin) : nat :=
  match b with
  | Zero => 0
  | D b' => mult (S (S 0)) (bin_to_nat b')
  | D_plus_1 b' => S (mult 2 (bin_to_nat b'))
  end.

Theorem increment_bin_to_nat_commute : forall (b : bin),
...
Proof.
  ...
  Case "b = D_plus_1 b'".
    ...
    rewrite -> IHb'. simpl. rewrite <- plus_n_Sm. reflexivity.
Qed.
(** [] *)
...
Theorem nat_bin_nat_identity : forall (n : nat),
  bin_to_nat (nat_to_bin n) = n.
Proof.
  intros n. induction n as [| n'].
  Case "n = 0". reflexivity.
  Case "n = S n'".
    simpl. rewrite -> increment_bin_to_nat_commute.
...
Fixpoint normalize (b : bin) : bin :=
  ...
  | D b' => match normalize b' with
            ...
            | _ => D (normalize b')
                 ...
                 (*| Basics.double b'' => Basics.double (Basics.double (normalize b''))
                 | Basics.double_plus_one b'' => 
                     Basics.double (Basics.double_plus_one (normalize b''))
                 end*)
  | D_plus_1 b' => D_plus_1 (normalize b')
  end.

Lemma double_bin_to_nat_eq_bin_to_nat_double : forall (b : bin),
  bin_to_nat (D b) = double (bin_to_nat b).
Proof.
  intros b. simpl. rewrite -> plus_0_r. rewrite <- double_plus.
  reflexivity.
Qed.

Lemma inc_nat_to_bin_eq_nat_to_bin_S : forall (n : nat),
  inc (nat_to_bin n) = nat_to_bin (S n).
Proof.
  intros n. reflexivity. Qed.

Lemma S_S_double_eq_double_S : forall (n : nat),
  S (S (double n)) = double (S n).
Proof.
  intros n. reflexivity. Qed.

Lemma inc_normalize_commute : forall (b : bin),
  inc (normalize b) = normalize (inc b).
Proof.
  intros b. induction b as [| b' | b'].
  Case "b = Zero". reflexivity.
  Case "b = D b'". simpl. destruct (normalize b') as [| b'' | b''].
    SCase "b' = Zero". reflexivity.
    SCase "b' = D b''". reflexivity.
    SCase "b' = D_plus_1 b'". reflexivity.
  Case "b = D_plus_1 b'". simpl. rewrite <- IHb'. destruct (normalize b').
    SCase "b' = Zero". reflexivity.
    SCase "b' = D b''". reflexivity.
    SCase "b' = D_plus_1 b''". reflexivity.
Qed.

Lemma nat_to_bin_double_eq_double_nat_to_bin : forall (n : nat),
  nat_to_bin (double n) = normalize (D (nat_to_bin n)).
Proof.
  intros n. induction n as [| n'].
  Case "n = 0". reflexivity.
  Case "n = S n'".
    simpl. rewrite -> IHn'. rewrite -> inc_normalize_commute.
    rewrite -> inc_normalize_commute. reflexivity.
Qed.
...
Lemma bin_to_nat_b_plus_b_eq_double_bin_to_nat_b : forall (b : bin),
  nat_to_bin (bin_to_nat b + bin_to_nat b) =
  normalize (D (nat_to_bin (bin_to_nat b))).
Proof.
  intros b. rewrite <- double_plus.
  rewrite <- double_bin_to_nat_eq_bin_to_nat_double.
  induction b as [| b' | b'].
  Case "b = Zero". reflexivity.
  Case "b = D b'". rewrite -> double_bin_to_nat_eq_bin_to_nat_double.
    rewrite -> nat_to_bin_double_eq_double_nat_to_bin. reflexivity.
  Case "b = D_plus_1 b'". simpl. rewrite -> plus_0_r.
    rewrite <- double_plus. rewrite -> plus_n_Sm. rewrite -> plus_assoc.
    rewrite <- double_plus. rewrite -> nat_to_bin_double_eq_double_nat_to_bin.
    rewrite <- IHb'. rewrite <- double_bin_to_nat_eq_bin_to_nat_double.
    rewrite <- inc_normalize_commute.
    rewrite -> double_bin_to_nat_eq_bin_to_nat_double.
    rewrite -> plus_comm. rewrite -> plus_1_l.
    rewrite -> inc_nat_to_bin_eq_nat_to_bin_S.
    rewrite -> S_S_double_eq_double_S.
    rewrite -> nat_to_bin_double_eq_double_nat_to_bin.
    rewrite -> inc_normalize_commute.
    simpl. reflexivity.
Qed.

Lemma normalize_normalize_eq_normalize : forall (b : bin),
  normalize (normalize b) = normalize b.
Proof.
  intros b. induction b as [| b' | b'].
  Case "b = zero". reflexivity.
  Case "b = D b'". induction (normalize (D b')) as [| b'' | b''].
    SCase "b' = zero". reflexivity.
    SCase "b' = double b''". simpl. rewrite -> IHb''.
Abort. (* :( *)  

Theorem bin_nat_bin_identity : forall (b : bin),
  nat_to_bin (bin_to_nat b) = normalize b.
Proof.
  intros b. induction b as [| b' | b'].
  Case "b = zero". reflexivity.
  Case "b = double b'". simpl. rewrite -> plus_0_r.
    rewrite -> bin_to_nat_b_plus_b_eq_double_bin_to_nat_b.
    rewrite -> IHb'.
Abort. 

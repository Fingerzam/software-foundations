...
Theorem silly_ex : 
     ...
     oddb 4 = true.
Proof.
  intros eq1. intros eq2. apply eq1. apply eq2. Qed.
(** [] *)
...
Theorem rev_exercise1 : forall (l l' : list nat),
     ...
     l' = rev l.
Proof.
  intros l l' eq. rewrite -> eq. symmetry. apply rev_involutive. Qed.
(** [] *)
...
Example sillyex1 : forall (X : Type) (x y z : X) (l j : list X),
     ...
     x = y.
Proof.
  intros X x y z l j. intros eq1 eq2. inversion eq2. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  Case "n = 0". intros m H. simpl in H.
    destruct m as [| m'].
    SCase "m = 0". reflexivity.
    SCase "m = S m'". inversion H.
  Case "n = S n'". intros m eq. simpl in eq.
    rewrite <- plus_n_Sm in eq. destruct m as [| m'].
    SCase "m = 0". inversion eq.
    SCase "m = S m'". simpl in eq. rewrite <- plus_n_Sm in eq.
...
Proof.
  ...
  Case "n = S n'". intros m eq. destruct m as [| m'].
    SCase "m = 0". inversion eq.
    SCase "m = S m'". simpl in eq. apply IHn' in eq. rewrite -> eq.
      reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  Case "l = []". intros n eq. reflexivity.
  Case "l = x :: l'".
    intros n eq. destruct n as [| n'].
    SCase "n = 0". inversion eq.
...
Theorem length_snoc''' : forall (n : nat) (X : Type) 
     ...
     length (snoc l v) = S n. 
Proof.
  intros n X v l. generalize dependent n. induction l as [| l'].
  Case "l = []". intros n eq. destruct n as [| n'].
    SCase "n = 0". reflexivity.
    SCase "n = S n'". inversion eq.
  Case "l = x :: l'". intros n eq. simpl. destruct n as [| n'].
    SCase "n = 0". inversion eq.
    SCase "n = S n'". simpl in eq. apply f_equal. apply IHl. inversion eq.
      reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  Case "l1 = []". intros l2 x n eq. simpl. simpl in eq. apply eq.
  Case "l1 = x' :: l'". intros l2 x n eq. destruct n as [| n'].
    SCase "n = 0". inversion eq.
    SCase "n = S n'". apply f_equal. simpl. simpl in eq.
      inversion eq. apply IHl1' in H0. rewrite -> H0.  inversion eq.
      reflexivity. Qed.
(** [] *)
...
Lemma length_app_cons : forall (X : Type) (x:X) (l1 l2 : list X),
  length (l1 ++ x :: l2) = S (length (l1 ++ l2)).
Proof.
  intros X x l1 l2. induction l1 as [| x' l1'].
  Case "l1 = []". reflexivity.
...
Proof.
  ...
  Case "l = []". intros n eq. destruct n as [| n'].
    SCase "n = 0". reflexivity.
    SCase "n = S n'". inversion eq.
  Case "l = x :: l'". intros n eq. destruct n as [| n'].
    SCase "n = 0". inversion eq.
    SCase "n = S n'". simpl. simpl in eq. rewrite <- plus_n_Sm.
      apply f_equal. rewrite -> length_app_cons. apply f_equal.
...
Theorem double_induction: forall (P : nat -> nat -> Prop), 
  ...
  forall m n, P m n.
Proof.
  intros P base step_dim1 step_dim2 step_diag. intros m.
  induction m as [| m'].
  Case "m = 0". intros n. induction n as [| n'].
    SCase "n = 0". apply base.
    SCase "n = S n'". apply step_dim2. apply IHn'.
  Case "m = S m'". intros n. induction n as [| n'].
    SCase "n = 0". apply step_dim1. apply IHm'.
    SCase "n = S n'". apply step_diag. apply IHm'.
Qed.
(** [] *)
...
Proof.
  ...
  Case "l = []". intros l1 l2 eq. simpl in eq. inversion eq.
    reflexivity.
  Case "l = p :: l'". intros l1 l2 eq. destruct p.
    destruct l1 as [| x' l1'].
    SCase "[]". unfold combine. inversion eq. destruct (split l').
      inversion H0.
    SCase "x' :: l1'". destruct l2 as [| y' l2'].
      SSCase "[]". simpl. inversion eq. destruct (split l').
        inversion H0.
      SSCase "y' :: l2'". simpl. simpl in eq. destruct (split l').
        inversion eq. apply f_equal. apply IHl'. rewrite -> H1.
...
Proof.
  ...
  Case "true". destruct (f true) eqn :Hft.
    SCase "true". rewrite -> Hft. rewrite -> Hft. reflexivity.
    SCase "false". destruct (f false) eqn: Hff.
      SSCase "true". rewrite -> Hft. reflexivity.
      SSCase "false". apply Hff.
  Case "false". destruct (f false) eqn: Hff.
    SCase "true". destruct (f true) eqn: Hft.
      SSCase "true". apply Hft.
      SSCase "false". apply Hff.
    SCase "false". rewrite -> Hff. apply Hff.
Qed.
(** [] *)
...
Proof.
  intros X x1 k1 k2 f eq. unfold override. destruct (beq_nat k1 k2) eqn: Heq.
    Case "true". apply beq_nat_true in Heq. rewrite <- Heq. symmetry. apply eq.
    Case "false". reflexivity.
Qed.
(** [] *)
...
Theorem beq_nat_trans : forall n m p,
  ...
  beq_nat n p = true.
Proof.
  intros n m p. intros eq1 eq2. apply beq_nat_true in eq1.
  apply beq_nat_true in eq2. rewrite -> eq1. rewrite -> eq2.
...
Proof.
  ...
  Case "l1 = []". intros l2. destruct l2 as [| y l2'].
    SCase "l2 = []". reflexivity.
    SCase "l2 = y :: l2'". intros eq. inversion eq.
  Case "l1 = x :: l1'". intros l2. destruct l2 as [| y l2'].
    SCase "l2 = []". intros eq. inversion eq.
    SCase "l2 = y :: l2'". intros eq.
      simpl in eq. inversion eq. apply IHl1' in H0.
      simpl. destruct (split (combine l1' l2')).
...
Proof.
  ...
  Case "true". destruct (beq_nat k2 k3) eqn: Hk2k3.
    SCase "true". rewrite -> beq_nat_sym in Hk2k3.
      assert (H: beq_nat k2 k1 = true).
      SSCase "proof of assertion". rewrite -> beq_nat_sym.
        apply beq_nat_trans with k3. apply Hk1k3. apply Hk2k3.
        rewrite -> H in eq. inversion eq.
    SCase "false". reflexivity.
  Case "false". destruct (beq_nat k2 k3) eqn: Hk2k3.
...
Proof.
  ...
  Case "l = x' :: l'". intros lf eq. simpl in eq.
    destruct (test x') eqn: Htest.
    SCase "true". inversion eq. rewrite <- H0.
      apply Htest.
    SCase "false". apply IHl' in eq. apply eq.
Qed.
(** [] *)
...
Definition forallb {X : Type} (pred : X -> bool) (l : list X) : bool :=
  fold (fun x b => andb (pred x) b) l true.

Definition existsb {X : Type} (pred : X -> bool) (l : list X) : bool :=
  fold (fun x b => orb (pred x) b) l false.

Definition existsb' {X : Type} (pred : X -> bool) (l : list X) : bool :=
  negb (forallb (complement pred) l).
...
Theorem existsb_eq_existsb' : forall (X : Type) (pred: X -> bool) (l : list X),
  existsb pred l = existsb' pred l.
Proof.
  intros X pred l. induction l as [| x l'].
  Case "l = []". reflexivity.
  Case "l = x :: l'". unfold existsb. unfold existsb'.
    simpl. unfold complement. unfold forallb. simpl.
    unfold existsb in IHl'. unfold existsb' in IHl'. unfold complement in IHl'.
    rewrite -> IHl'. destruct (pred x).
    SCase "true". reflexivity.
    SCase "false". reflexivity.
Qed.
(** [] *)

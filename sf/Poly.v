...
(* combine : forall (X : Type) (Y : Type), list X -> List Y -> List (X*Y) *)
...
Definition complement {X : Type} (predicate : X -> bool) (x : X) :=
  negb (predicate x).

Definition partition {X : Type} (test : X -> bool) (l : list X)
                     : list X * list X :=
  (filter test l, filter (complement test) l).
...
Theorem fold_length_correct : forall X (l : list X),
  ...
  intros X l. unfold fold_length. induction l as [| x l'].
  Case "l = []". reflexivity.
...
Proof.
  intros X Y f l. induction l as [| x l'].
  Case "l = []". reflexivity.
  Case "l = x :: l'". unfold fold_map. simpl.
    replace (fold (fun (x0 : X) (l'0 : list Y) => f x0 :: l'0) l' [])
    with (fold_map f l'). rewrite ->  IHl'. reflexivity.
    unfold fold_map. reflexivity. Qed.
(** [] *)

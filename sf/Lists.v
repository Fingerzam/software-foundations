...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | [] => true
  | n :: s1' => match member n s2 with
                | true => subset s1' (remove_one n s2)
                | false => false
                end
  end.
...
Theorem bag_add_increments_count : forall (n : nat) (s : bag),
  count n (add n s) = S (count n s).
Proof.
  intros n s. simpl. rewrite <- beq_nat_refl. reflexivity.
Qed.
(** [] *)
...
Lemma snoc_l_n_eq_app_l_n : forall (l : natlist) (n : nat),
  snoc l n = l ++ [n].
Proof.
...
Lemma snoc_rev_eq_rev_cons : forall (n : nat) (l : natlist),
  snoc (rev l) n = rev (n :: l).
Proof.
  intros n l. reflexivity.
Qed.

Lemma rev_snoc_eq_cons_rev : forall (n : nat) (l : natlist),
...
Proof.
  ...
  Case "l = []". reflexivity.
  Case "l = n :: l'".
    simpl. rewrite -> rev_snoc_eq_cons_rev.
...
Theorem snoc_append : forall (l:natlist) (n:nat),
  snoc l n = l ++ [n].
Proof.
  intros l n. rewrite -> snoc_l_n_eq_app_l_n. reflexivity.
Qed.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  ...
  | _ , [] => false
  | [],  _ => false
  | (n1 :: l1'), (n2 :: l2') => andb (beq_nat n1 n2)
                                     (beq_natlist l1' l2')
  end.
...
Proof.
  intros l. induction l as [| n l'].
  Case "l = []". reflexivity.
  Case "l = n :: l'". simpl. rewrite <- beq_nat_refl. rewrite <- IHl'.
    simpl. reflexivity.
Qed.
(** [] *)
...
Theorem asdf : forall (n1 n2 : nat) (l1 l2 : natlist),
  n1 :: (snoc (l1 ++ l2) n2) = [n1] ++ l1 ++ l2 ++ [n2].
Proof.
  intros n1 n2 l1 l2. simpl. rewrite -> snoc_append.
...
Theorem count_bag_sum_eq_count_plus_count : forall (n : nat) (s1 s2 : bag),
  count n (sum s1 s2) = (count n s1) + (count n s2).
Proof.
  intros n s1 s2. induction s1 as [| n' s1'].
  Case "s1 = []". reflexivity.
  Case "s1 = n' :: s1'". simpl. destruct (beq_nat n n').
    SCase "true". simpl. rewrite -> IHs1'. reflexivity.
    SCase "false". rewrite -> IHs1'. reflexivity.
Qed.
(** [] *)

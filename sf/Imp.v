...
Fixpoint optimize_bools (b : bexp) : bexp :=
  match b with
  | BAnd b1 b2 => match (optimize_bools b1), (optimize_bools b2) with
                  | BFalse, _ => BFalse
                  | _, BFalse => BFalse
                  | BTrue, b' => b'
                  | b', BTrue => b'
  ...
  | BNot b1 => match optimize_bools b1 with
               | BTrue => BFalse
               | BFalse => BTrue
               | b1' => BNot b1'
               end
...
Theorem optimize_bools_sound : forall b,
  beval (optimize_bools b) = beval b.
Proof.
  intros b.
  bexp_cases (induction b) Case;
  ...
  Case "BNot".
    simpl.
    bexp_cases (destruct (optimize_bools b)) SCase;
      try (rewrite <- IHb; simpl;reflexivity).
    SCase "BAnd".
      simpl. rewrite <- IHb1. rewrite <- IHb2.
      bexp_cases (destruct (optimize_bools b1)) SSCase;
        try reflexivity;
        try (destruct (optimize_bools b2); try reflexivity);
        try (rewrite andb_comm; reflexivity). Qed.
(** [] *)
...
Tactic Notation "bevalR_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "E_BAnd" | Case_aux c "E_BNot" ].
...
Proof.
  ...
  Case "<-".
    intros H. generalize dependent b.
    bexp_cases (induction e) SCase; simpl; intros;
      try (subst; constructor; apply aeval_iff_aevalR; reflexivity).
      SCase "BNot".
        subst. constructor. apply IHe. reflexivity.
      SCase "BAnd".
        subst. constructor; try apply IHe1; try apply IHe2; reflexivity. Qed.
...
Theorem update_eq : forall n x st,
  (update st x n) x = n.
Proof.
  intros n x st. unfold update. apply eq_id. Qed.
(** [] *)
...
Proof.
  intros x2 x1 n st Hneq. apply neq_id.
  apply Hneq. Qed.
(** [] *)
...
Proof.
  intros n. apply neq_id. intros H. inversion H. Qed.
(** [] *)

Theorem neq_sym : forall (X : Type) (x y : X),
  x <> y -> y <> x .
Proof.
  intros X x y. intros Hneq. unfold not. intros Heq.
  apply eq_sym in Heq. apply Hneq in Heq. inversion Heq. Qed.
...
Proof.
  ...
  Case "x1 = x2".
    rewrite -> Heq. rewrite update_eq. rewrite update_eq. reflexivity.
  Case "x1 <> x2".
    repeat (rewrite update_neq);
      try reflexivity;
      try (apply neq_sym; apply Hneq). Qed.
(** [] *)
...
Proof.
  ...
  Case "x1 <> x2".
    apply update_neq. apply Hneq. Qed.
(** [] *)
...
Proof.
  intros n1 n2 x1 x2 x3 st Hneq.
  destruct (eq_id_dec x1 x3) as [Heq13 | Hneq13].
  Case "x1 = x3".
    destruct (eq_id_dec x2 x3) as [Heq23 | Hneq23].
    SCase "x2 = x3".
      assert (Heq: x2 = x1). apply eq_trans with x3. apply Heq23. apply eq_sym.
        apply Heq13.
      apply Hneq in Heq. inversion Heq.
    SCase "x2 <> x3".
      rewrite Heq13. unfold update. rewrite eq_id. rewrite neq_id.
      rewrite eq_id. reflexivity.
      apply Hneq23.
  Case "x1 <> x3".
    destruct (eq_id_dec x2 x3) as [Heq23 | Hneq23].
    SCase "x2 = x3".
      rewrite Heq23. rewrite update_neq. rewrite update_eq. rewrite update_eq.
      reflexivity. apply Hneq13.
    SCase "x2 <> x3".
      repeat (rewrite update_neq);
        try reflexivity;
        try assumption. Qed.
...
Proof.
  ...
  inversion Heval.
  subst. simpl. apply update_eq. Qed.
(** [] *)
...
Inductive no_whilesR: com -> Prop :=
  ...
  | nw_ass : forall id ae, no_whilesR (id ::= ae)
...
Proof.
  intros c. split.
  Case "->".
    com_cases (induction c) SCase; intros H.
    SCase "SKIP".
      apply nw_skip.
    SCase "::=".
      apply nw_ass.
    SCase ";;".
      simpl in H. apply andb_true_iff in H. inversion H. apply nw_seq.
        apply IHc1. assumption.
        apply IHc2. assumption.
    SCase "IFB".
      simpl in H. apply andb_true_iff in H. inversion H. apply nw_if.
        apply IHc1. assumption.
        apply IHc2. assumption.
    SCase "WHILE".
  ...
  Case "<-".
    ...
    SCase ";;".
      ...
      split; assumption.
    SCase "IFB".
      simpl. apply andb_true_iff. split; assumption. Qed.
(** [] *)
...
Theorem no_whiles__terminates : forall (c : com) (st : state),
...
Proof.
  ...
  Case "::=".
    exists (update st id (aeval st ae)).
    apply E_Ass. reflexivity.
  Case ";;".
    assert (exists st2 : state, c1 / st || st2) as Hst'. apply IHno_whilesR1.
    inversion Hst' as [st2].
    assert (exists st3 : state, c2 / st2 || st3) as Hst''. apply IHno_whilesR2.
    inversion Hst'' as [st3].
    exists st3. apply E_Seq with st2. apply H1. apply H2.
  Case "IFB".
    destruct (beval st b) eqn: Hb.
    SCase "true".
      assert (exists st' : state, ct / st || st') as Hst'.
        apply IHno_whilesR1.
      inversion Hst' as [st'].
      exists st'. apply E_IfTrue. assumption. assumption.
    SCase "false".
      assert (exists st' : state, cf / st || st') as Hst'.
        apply IHno_whilesR2.
      inversion Hst' as [st'].
      exists st'. apply E_IfFalse. assumption. assumption. Qed.
(** [] *)
...
Lemma s_compile_lemma : forall e state stack prog,
  s_execute state stack (s_compile e ++ prog) =
  s_execute state (s_execute  state stack (s_compile e)) prog.
Proof.
  intros e.
  aexp_cases (induction e) Case;
    intros state stack prog; try reflexivity; try simpl;
    try (
      repeat (rewrite <- app_assoc);
      rewrite IHe1; rewrite IHe2; simpl;
      destruct (s_execute state (s_execute state stack (s_compile e1)) (s_compile e2))
        as [| n1 stack1] eqn:H;
        [rewrite IHe1; rewrite IHe2; rewrite H; reflexivity |
         destruct stack1 as [| n2 stack2];
          rewrite IHe1; rewrite IHe2; rewrite H; reflexivity]). Qed.

Lemma s_compile_lemma2 : forall (st : state) (e : aexp) (stack : list nat),
...
Proof.
  ...
  aexp_cases (induction e) Case;
    ...
    try reflexivity;
    try simpl;
    try rewrite s_compile_lemma;
    try rewrite s_compile_lemma;
    try (rewrite IHe1; rewrite IHe2; reflexivity). Qed.

Theorem s_compile_correct : forall (st : state) (e : aexp),
  s_execute st [] (s_compile e) = [ aeval st e ].
Proof.
  intros st e.
  apply s_compile_lemma2 with (st:=st) (stack:=[]) (e:=e). Qed.
(** [] *)
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_SeqBreak : forall st st' c1 c2,
      c1 / st || SBreak / st' ->
      (c1 ; c2) / st || SBreak / st'
  | E_SeqContinue : forall st st' st'' c1 c2 s,
      c1 / st || SContinue / st' ->
      c2 / st' || s / st'' ->
      (c1 ; c2) / st || s / st''
...
Tactic Notation "ceval_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "E_Ass" | Case_aux c "E_SeqBreak"
  | Case_aux c "E_Seq_Continue" | Case_aux c "E_IfTrue"
  | Case_aux c "E_IfFalse" | Case_aux c "E_WhileEnd"
  | Case_aux c "E_WhileLoopBreak" | Case_aux c "E_WhileLoopContinue"
  ].
...
Theorem break_ignore : forall c st st' s,
     ...
     st = st'.
Proof.
  intros c st st' s. intros seq. inversion seq.
...
Theorem while_continue : forall b c st st' s,
  ...
  s = SContinue.
Proof.
  intros b c st st' s loop.
  inversion loop; try reflexivity. Qed.
...
Theorem ceval_deterministic: forall (c:com) st st1 st2 s1 s2,
     ...
     st1 = st2 /\ s1 = s2.
Proof.
  intros c  st st1 st2 s1 s2 Ec1 Ec2. 
  generalize dependent st2.
  generalize dependent s2.
  ceval_cases (induction Ec1) Case;
    intros s2 st2 Ec2;
    inversion Ec2;
  ...
  Case "E_SeqBreak".
    apply IHEc1. apply H4. apply IHEc1 in H1.
    inversion H1. inversion H7.
  Case "E_Seq_Continue".
    SCase "s2 = SBreak".
      apply IHEc1_1 in H4. inversion H4. inversion H6.
    SCase "s2 = SContinue".
      apply IHEc1_1 in H1. inversion H1.
      rewrite <- H6 in H5. apply IHEc1_2 in H5.
      apply H5.
  Case "E_IfTrue".
    SCase "b ~> true".
      apply IHEc1 in H6.
  ...
  Case "E_WhileLoopBreak".
    SCase "b / st2 || false".
      rewrite <- H4 in H5. rewrite H5 in H. inversion H.
    SCase "b / st || true".
      apply IHEc1 in H6.
      inversion H6.
      split; try reflexivity.
      apply H7.
    SCase "b / st || true /\ c / st || SContinue / st'0".
      clear H2. split; try reflexivity.
      apply IHEc1 in H3. inversion H3. inversion H8.
  Case "E_WhileLoopContinue".
    SCase "b / st2 || false".
      rewrite <- H4 in H5. rewrite H5 in H. inversion H.
    SCase "c / st || SBreak / st2".
      apply IHEc1_1 in H6. inversion H6. inversion H8.
    SCase "c / st || SContinue / st'0".
      apply IHEc1_1 in H3. inversion H3.
      split; try reflexivity.
      rewrite <- H8 in H7. apply IHEc1_2 in H7.
      inversion H7. apply H10. Qed.
End BreakImp.
(** [] *)
